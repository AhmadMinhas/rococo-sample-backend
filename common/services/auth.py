...
class AuthService:
    ...
    def generate_reset_password_token(self, login_method: LoginMethod, email: str):
        return self.generate_token(login_method, email, 'reset_password')


    def generate_email_verification_token(self, login_method: LoginMethod, email: str):
        return self.generate_token(login_method, email, 'email_verification')
    ...
    @staticmethod
    def parse_verify_token(token, login_method: LoginMethod):
        try:
            decoded = jwt.decode(token, login_method.password, algorithms=['HS256'])
            exp_time = decoded['exp']
            if not decoded['token_type'] == 'email_verification':
                return
            if time.time() <= exp_time:
                return decoded
        except jwt.ExpiredSignatureError:
    ...
    def verify_email(self, token: str, uidb64: str):
        ...
        if not login_method:
            raise APIException("Invalid email verification Link.")

        parsed_token = self.parse_verify_token(token, login_method)
